...
from tests.data.test_data import sample_texts
...
class TestTransformationEnginePerformance:
    ...
    @pytest.mark.load
    def test_single_transformation_performance(self, transformer):
        ...
        # Test different transformations for performance
        transformations = ["alternate_case", "backwards", "l33t_speak", "rot13"]
    ...
    @pytest.mark.load
    def test_large_text_transformation_performance(self, transformer):
        ...
        for size in sizes:
            ...
            assert duration < expected_max_time, f"Large text ({size} chars) took {duration:.4f}s"
...
class TestAPIEndpointPerformance:
    ...
    def test_transform_endpoint_performance(self, app):
        ...
        with app.test_client() as client:
            request_data = {"text": "Performance test", "transformation": "alternate_case"}
...
class TestConcurrentLoadPerformance:
    ...
    def test_concurrent_mixed_requests(self, app):
        ...
        def transform_requests():
            ...
            with app.test_client() as client:
                for i in range(15):
                    request_data = {"text": f"Concurrent test {i}", "transformation": "backwards"}
        ...
        assert total_duration < 5.0, f"Mixed concurrent requests took {total_duration:.4f}s"
        
        avg_duration = total_duration / total_requests
        assert avg_duration < 0.2, f"Average concurrent request time {avg_duration:.4f}s"
