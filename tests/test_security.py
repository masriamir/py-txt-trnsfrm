...
class TestSecurityMeasures:
    ...
    def test_no_hardcoded_secrets(self):
        ...
        for config in configs:
            ...
            if hasattr(config, 'SECRET_KEY') and config.SECRET_KEY:
                assert config.SECRET_KEY != 'dev', "SECRET_KEY should not be 'dev' in production"
                assert len(config.SECRET_KEY) > 10, "SECRET_KEY should be sufficiently long"
    ...
    def test_transformation_output_sanitization(self):
        ...
        for dangerous_input in dangerous_inputs:
            for transform in ['alternate_case', 'backwards', 'rot13']:
                ...
                if transform == 'rainbow_html' and '<script>' in dangerous_input.lower():
    ...
    def test_security_scan_results(self):
        ...
        if bandit_results:
            ...
            high_severity_issues = [
                ...
                if issue.get('issue_confidence') == 'HIGH' and
                   issue.get('issue_severity') == 'HIGH'
            ]

            # Fail if there are critical security issues
            assert len(high_severity_issues) == 0, f"Found {len(high_severity_issues)} high-severity security issues"

        if safety_results and isinstance(safety_results, list):
            # Fail if there are any known vulnerabilities
            assert len(safety_results) == 0, f"Found {len(safety_results)} known vulnerabilities in dependencies"
    ...
    def test_no_debug_in_production(self):
        ...
        assert not getattr(ProductionConfig, 'DEBUG', False), "Debug should be False in production"
        assert not getattr(ProductionConfig, 'TESTING', False), "Testing should be False in production"
