...
from app.env_config import FlaskEnvironment, LogLevel, LoggingConfig
...
class TestTextTransformationWorkflow:
    ...
    @pytest.mark.integration
    def test_multiple_transformation_types_workflow(self):
        ...
        with app.test_client() as client:
            for text, transformation in transformations_to_test:
                response = client.post("/transform", 
                                     json={"text": text, "transformation": transformation})
    ...
    @pytest.mark.integration
    def test_edge_case_transformations_workflow(self):
        ...
        with app.test_client() as client:
            for text, transformation in edge_cases:
                response = client.post("/transform", 
                                     json={"text": text, "transformation": transformation})
...
class TestRequestResponseWorkflow:
    ...
    def test_session_context_workflow(self):
        ...
        with app.test_client() as client:
            ...
            response3 = client.post("/transform", 
...
class TestConfigurationIntegrationWorkflow:
    ...
    @pytest.mark.integration
    def test_environment_based_configuration_workflow(self):
        """Test configuration workflow based on environment."""
        # Test different environments produce different behaviors
        configs = [
            (DevelopmentConfig, True, False),   # debug=True, testing=False
            (TestConfig, False, True),          # debug=False, testing=True
        ]
        
        for config_class, expected_debug, expected_testing in configs:
...
class TestEndToEndWorkflow:
    ...
    def test_data_consistency_workflow(self):
        ...
        with app.test_client() as client:
            for text, transformation, expected_start in test_data:
                response = client.post("/transform", 
                                     json={"text": text, "transformation": transformation})
